...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if ((doctor.unavailableDates ?? []).includes(date)) return err(`already marked unavailable for ${date}`, 409);

  const dates = [...(doctor.unavailableDates ?? []), date];
  db.update(schema.doctors).set({ status: "unavailable", unavailableDates: dates }).where(eq(schema.doctors.id, doctor.id)).run();
  audit({ actor: "doctor", action: "doctor.marked_unavailable", refType: "doctor", refId: doctor.id, detail: { date, reason: b.reason } });
  ...
  const eventId = enqueueEvent("doctor_emergency", { doctorId: doctor.id, date, reason: b.reason ?? "emergency" });
  return json({ ok: true, date, calendar: calendarLabel, eventId });
}
