...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (service === "calendar") {
      ...
      if (!auth) return json({ ok: false, detail: "Google account not connected (run OAuth first)" });
      const provider = new GoogleCalendarProvider();
      const santos = db.select().from(schema.doctors).all().find((d) => d.calendarId && !d.calendarId.startsWith("sim-"));
      ...
      setServiceHealth("calendar", { status: "ok", detail: `verified (${Date.now() - started}ms)` });
      return json({ ok: true, detail: `Read ${events.length} event(s) from "${calId}" over the next 7 days.` });
    }
    if (service === "gmail") {
      const auth = authorizedClient();
      if (!auth) return json({ ok: false, detail: "Google account not connected (run OAuth first)" });
      ...
      setServiceHealth("mail", { status: "ok", detail: `verified (${Date.now() - started}ms)` });
      return json({ ok: true, detail: `Gmail connected as ${profile.emailAddress}.` });
    }
    if (service === "mcp") {
      const status = await runMcpHealthCheck();
      return json({ ok: status.state === "connected", detail: status.detail, state: status.state, tools: status.state === "connected" ? status.tools : [] });
    }
    return err("service must be gemini | calendar | gmail | mcp");
  } catch (e) {
    const detail = String((e as Error).message ?? e).slice(0, 300);
    if (service === "gemini") setServiceHealth("gemini", { status: "error", detail });
    if (service === "calendar") setServiceHealth("calendar", { status: "error", detail });
    if (service === "gmail") setServiceHealth("mail", { status: "error", detail });
    return json({ ok: false, detail });
  } finally {
    audit({ actor: "staff", action: "integration.verify", detail: { service } });
  }
}
