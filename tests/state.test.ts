...
import { openCase, transitionCase, TransitionError, getCase, maybeResolveCase, escalateCase } from "@/core/cases";
import { db, schema } from "@/core/db/client";
import { eq } from "drizzle-orm";

function newCase() {
  return openCase({ clinicId: "clinic_riverside", type: "doctor_emergency", severity: "high", title: "test case", meta: {} });
}
...
describe("case state machine", () => {
  ...
  it("rejects illegal jumps", () => {
    const c = newCase();
    expect(() => transitionCase(c.id, "executing", "staff", "skip ahead")).toThrow(TransitionError);
    expect(() => transitionCase(c.id, "resolved", "system", "skip ahead")).toThrow(TransitionError);
  });
  ...
  it("only staff can move awaiting_approval → executing", () => {
    ...
    transitionCase(c.id, "planning", "orchestrator", "t");
    transitionCase(c.id, "awaiting_approval", "orchestrator", "t");
    expect(() => transitionCase(c.id, "executing", "orchestrator", "agent tries")).toThrow(TransitionError);
    expect(() => transitionCase(c.id, "executing", "executor", "executor tries")).toThrow(TransitionError);
  ...
  it("maybeResolveCase waits for every substantive outcome", () => {
    ...
    db.insert(schema.recommendations)
      .values({ caseId: c.id, kind: "reschedule", status: "executed", outcome: "pending", payload: {}, createdAt: new Date().toISOString() })
      .run();
    transitionCase(c.id, "awaiting_approval", "orchestrator", "t");
    transitionCase(c.id, "executing", "staff", "t");
    transitionCase(c.id, "resolving", "executor", "t");
    ...
    const rec = db.select().from(schema.recommendations).where(eq(schema.recommendations.caseId, c.id)).get()!;
