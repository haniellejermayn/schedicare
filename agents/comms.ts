...
import { ReplyInterpretationSchema, type ReplyInterpretation } from "@/core/types";
...
export const DRAFT_PURPOSES = ["reschedule_offer", "confirm_nudge", "preventive", "waitlist_offer", "cancel_ack"] as const;
...
function template(purpose: DraftPurpose, item: DraftItem): { subject: string; body: string } {
 ...
 */
const BANNED = /(diagnos\w*|dosage|\bmg\b|prescri\w*|take your medication|stop taking|side effects?|symptom)/i;
...
export function bannedContentLint(
  ...
  const drafts = result.drafts.map((d) => {
    if (BANNED.test(d.subject) || BANNED.test(d.body)) {
      ...
      warnings.push(`Draft for ${item?.patientName ?? d.patientId} contained clinical language and was replaced with the standard template.`);
      const safe = item ? template(purpose, item) : { subject: d.subject, body: "" };
...
export const commsDraftAgent: AgentDef<CommsDraftInput, CommsDraftResult> = {
  name: "comms",
  feedVerb: (i) => `Drafting ${i.items.length} patient message${i.items.length === 1 ? "" : "s"}`,
...
- One draft per item, matching patientId/appointmentId. Finish with submit_result.`,
  ...
  buildPrompt: (i) =>
    ...
    i.items.map((it) => `- ${it.patientName} (${it.patientId}${it.appointmentId ? `, appt ${it.appointmentId}` : ""}): ${JSON.stringify(it.context)}`).join("\n"),
  fallback: async (i) => ({ drafts: i.items.map((it) => ({ patientId: it.patientId, appointmentId: it.appointmentId, ...template(i.purpose, it) })) }),
};
...
export function guardReply(body: string): { hit: boolean; reason?: string } {
  ...
  const anger = /(furious|angry|outrag\w*|unacceptable|lawyer|sue you|complaint|worst (clinic|service)|scam|report you)/i;
  if (medical.test(body)) return { hit: true, reason: "Reply contains medical content — a person must read it." };
  if (injection.test(body)) return { hit: true, reason: "Reply contains instruction-like content and was quarantined for human review." };
  if (anger.test(body)) return { hit: true, reason: "Patient sounds upset — routing to a person rather than automation." };
  return { hit: false };
}
...
export function ruleClassifyReply(body: string): ReplyInterpretation {
  ...
  const acceptRe = /(^| )(yes|yep|yup|sure|confirm(ed)?|i'?ll be there|see you (then|there)|that works|works (for me|great|fine)|sounds good|i('| a)m coming|take it|i'?ll take)/;
  const negationCue = /(doesn'?t work|does not work|can'?t (make|do)|cannot (make|do)|won'?t work|not (available|possible)|no longer|unfortunately|but |\b(pero|hindi|baka|ewan)\b|not sure)/;
  if (acceptRe.test(t) && !negationCue.test(t)) {
    return { intent: "accept_offer", confidence: 0.9, summary: "Patient accepted the offered time." };
  }
  ...
  const dayPart = /\b(morning)s?\b/.test(t) ? "am" : /\b(afternoon)s?\b/.test(t) ? "pm" : undefined;
  const weekday = WEEKDAYS.find((w) => t.includes(w));
  const asksDifferent = /(another (time|day|slot)|different (time|day)|reschedul|earlier|later|next week|instead)/.test(t);
  if (after || before || afterNoonish || beforeNoonish || dayPart || weekday || asksDifferent) {
    const constraint: ReplyInterpretation["constraint"] = {};
    if (after) constraint.afterTime = to24h(Number(after[1]), Number(after[2] ?? 0), after[3] as string | undefined);
    if (before) constraint.beforeTime = to24h(Number(before[1]), Number(before[2] ?? 0), before[3] as string | undefined);
    if (afterNoonish && !constraint.afterTime) constraint.afterTime = "13:00";
    if (beforeNoonish && !constraint.beforeTime) constraint.beforeTime = "12:00";
    ...
    const summaryBits = [
      ...
      constraint.dayPart && (constraint.dayPart === "am" ? "mornings" : "afternoons"),
  ...
  if (/(cancel (my|the|this)|please cancel|won'?t be (coming|able)|need to cancel)/.test(t)) {
    return { intent: "cancel", confidence: 0.9, summary: "Patient wants to cancel." };
  }
  if (/(doesn'?t work|does not work|can'?t (make|do)|not available|none of (these|those)|won'?t work|no,? (thanks|thank you)|decline)/.test(t)) {
    return { intent: "reject_offer", confidence: 0.85, summary: "Patient declined the offered time." };
  }
  if (/\?$/.test(t) || /^(what|when|where|how|who|is |are |do |does |can you tell)/.test(t)) {
    return { intent: "question", confidence: 0.7, summary: "Patient asked a question — staff should reply." };
  }
  return { intent: "needs_human", confidence: 0.5, summary: "Could not classify the reply with confidence — routing to staff." };
}
...
export const commsInterpretAgent: AgentDef<CommsInterpretInput, ReplyInterpretation> = {
  name: "comms",
