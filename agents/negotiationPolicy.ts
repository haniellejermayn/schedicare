...
import type { ReplyRegister } from "./comms";
...
- Zero candidates → ask_clarification. Target the relaxation with the best realistic yield: highest slotsIfDropped, but weigh plausibility — a patient who wrote "only Dr. Santos, she knows my history" is likelier to flex on time than on doctor. The question may cite ONLY provided counts, phrased the way a front desk would: use the exact number only when it is 5 or fewer (e.g. "if afternoons could work, we have 4 options this week"); for larger counts use qualitative wording ("we have several openings on weekday afternoons") — big precise numbers read as robotic, and the exact figures already sit on the staff card. Warm and brief (≤3 sentences).
...
- Language is supplied only as replyRegister. english: warm, plain English. taglish: natural Filipino clinic communication with a simple English/Taglish base and natural uses of "po"; avoid translated-sounding or formal Filipino. tagalog: conversational Filipino, never stiff or ceremonial. Never imitate slang, anger, misspellings, or excessive informality. Avoid "ipinababatid", "makipag-ugnayan", and "kung inyong nanaisin".
- When requestedTargetField is supplied, ask_clarification about exactly that field; do not choose another relaxation or offer a slot.
- For requireSameDoctor, use natural front-desk wording such as: "Wala pong available slot with Dr. Santos at that time. If okay po sa inyo, pwede po namin kayo i-assign sa ibang doctor na available. Just let us know po." Use the supplied originalDoctorName. Never say "if you insist" or "redirect".
...
export const negotiationPolicyAgent: AgentDef<PolicyInput, NegotiationAction> =
  {
    ...
    buildPrompt: (i) => {
      ...
      return (
        ...
        `Current doctor: ${i.originalDoctorName ?? "not provided"}\n` +
        `Round: ${i.turn + 1} of ${i.turnBudget}\n\n` +
        `Reply register: ${i.replyRegister ?? "english"}\n` +
        `Requested relaxation: ${i.requestedTargetField ?? "choose the best computed option"}\n\n` +
