...
function caseExistsFor(type: string, appointmentId: string): boolean {
  const rows = db
    ...
    .all();
  return rows.some((r) => ((r.meta as any)?.appointmentId ?? "") === appointmentId);
}
...
export async function runDailySweep(): Promise<void> {
  ...
  audit({ actor: "system", action: "sweep.started", detail: { day: demoToday() } });

  // 1) Unconfirmed within 36h → confirmation case.
  const active = db
    .select()
    .from(schema.appointments)
    .where(and(eq(schema.appointments.status, "booked"), gt(schema.appointments.startUtc, demoNow().toISOString())))
    .all();
  for (const a of active) {
    ...
    if (a.startUtc.slice(0, 10) === demoNow().toISOString().slice(0, 10) || localDay(a.startUtc) === demoToday()) continue;
  ...
  const cancelled = db
    .select()
    .from(schema.appointments)
    .where(and(inArray(schema.appointments.status, ["cancelled_by_patient"]), gt(schema.appointments.startUtc, demoNow().toISOString())))
    .all();
