...
import { runScheduling, searchWindow, type SchedulingRequest, type SchedulingResult } from "@/agents/scheduling";
import { runRecovery, runWaitlistFill, type RecoveryItemInput, type RecoveryResult, type WaitlistFillResult } from "@/agents/recovery";
import { bannedContentLint, runCommsDraft, type CommsDraftResult, type DraftItem, type DraftPurpose } from "@/agents/comms";
...
export async function assessStep(caseId: string): Promise<string> {
  ...
  if (c.state === "open") transitionCase(caseId, "assessing", "orchestrator", "Assessment starting");
  const doctor = getDoctor(m.doctorId);
  await pace();
  const res = await runAssessment(
    { caseId, doctorId: m.doctorId, doctorName: doctor.name, date: m.date, reason: m.reason ?? "emergency" },
    { caseId }
  );
  const a: AssessmentResult = res.output;
  db.update(schema.cases).set({ severity: a.severity }).where(eq(schema.cases.id, caseId)).run();
  ...
  if (getCase(caseId).state === "assessing") transitionCase(caseId, "planning", "orchestrator", "Assessment complete — planning recovery");
  await pace();
  return a.summary;
}
...
export async function scheduleStep(
  caseId: string,
  constraints?: { appointmentId?: string; afterTime?: string; beforeTime?: string; dayPart?: "am" | "pm"; preferredDay?: string }
): Promise<string> {
  const m = meta(caseId);
  const assessment: AssessmentResult | undefined = m.assessment;
  const items = (assessment?.items ?? []).filter(
    (it: any) => !constraints?.appointmentId || it.appointmentId === constraints.appointmentId
  );
  if (items.length === 0) throw new Error("scheduleStep: no assessed items to search for");

  const win = constraints?.preferredDay ? { fromDay: constraints.preferredDay, toDay: searchWindow(constraints.preferredDay, 4).toDay } : searchWindow(demoToday(), 8);
  const requests: SchedulingRequest[] = items.map((it: any) => ({
    appointmentId: it.appointmentId,
    patientId: it.patientId,
    type: it.type,
    originalDoctorId: m.doctorId ?? it.originalDoctorId ?? getAppt(it.appointmentId).doctorId,
  ...
  if (dropped > 0)
    timeline(caseId, "scheduling", "error", `${dropped} proposed option${dropped === 1 ? "" : "s"} failed re-validation and were dropped`, "Options must pass the deterministic placement validator; agents cannot override it.");
  updateCaseMeta(caseId, { slotOptions: validated, searchSummary: out.searchSummary, searchConstraints: constraints ?? null });
  timeline(caseId, "scheduling", "status", "Valid slot options collected", out.searchSummary, { mode: res.mode });
  await pace();
  return out.searchSummary;
}

function getAppt(id: string) {
  const a = db.select().from(schema.appointments).where(eq(schema.appointments.id, id)).get();
 ...
 */
function dedupeChosenAcrossPatients(r: RecoveryResult, items: RecoveryItemInput[], caseId: string): void {
  const priority = new Map(items.map((it) => [it.appointmentId, it.priorityRank]));
  const nameOf = new Map(items.map((it) => [it.appointmentId, it.patientName]));
  const ordered = [...r.plans].sort((a, b) => (priority.get(a.appointmentId) ?? 99) - (priority.get(b.appointmentId) ?? 99));
  const keyOf = (o: { doctorId: string; startUtc: string }) => `${o.doctorId}|${o.startUtc}`;
  ...
  for (const plan of ordered) {
    ...
    const chosen = plan.options.find((o) => o.id === plan.chosenOptionId) ?? plan.options[0];
    if (claimed.has(keyOf(chosen))) {
      const alt = plan.options.find((o) => !claimed.has(keyOf(o)));
      if (alt) {
        plan.chosenOptionId = alt.id;
        const holder = nameOf.get(claimed.get(keyOf(chosen))!) ?? "another patient";
        plan.reorderReason = [plan.reorderReason, `Top slot shifted to avoid double-booking with ${holder}.`]
          .filter(Boolean)
          .join(" ")
          .slice(0, 200);
        timeline(caseId, "recovery", "status", `${nameOf.get(plan.appointmentId) ?? plan.appointmentId}: shifted to the next open option to avoid a double-booking`);
        claimed.set(keyOf(alt), plan.appointmentId);
      } else {
        plan.chosenOptionId = "none";
        plan.options = [];
        plan.rationale = `${plan.rationale} All ranked options were claimed by higher-priority patients; escalating to staff.`.slice(0, 400);
      }
    } else {
...
export async function recoverStep(caseId: string, onlyAppointmentId?: string): Promise<string> {
  const m = meta(caseId);
  const assessment: AssessmentResult | undefined = m.assessment;
  const slotOptions: Record<string, Slot[]> = m.slotOptions ?? {};
  const items: RecoveryItemInput[] = (assessment?.items ?? [])
    .filter((it: any) => !onlyAppointmentId || it.appointmentId === onlyAppointmentId)
    .map((it: any) => ({
      appointmentId: it.appointmentId,
      patientId: it.patientId,
  ...
  timeline(caseId, "recovery", "status", `Recovery plans ranked for ${r.plans.length} patient${r.plans.length === 1 ? "" : "s"}`, r.summary, { mode: res.mode });
  await pace();
  return r.summary;
}
...
export async function commsStep(caseId: string, opts?: { replanOf?: string; replanNote?: string; onlyAppointmentId?: string }): Promise<string> {
  const m = meta(caseId);
  const assessment: AssessmentResult | undefined = m.assessment;
  const plans = (m.plans ?? []).filter((p: any) => !opts?.onlyAppointmentId || p.appointmentId === opts.onlyAppointmentId);
  if (plans.length === 0) throw new Error("commsStep: no plans to draft for");

  const items: DraftItem[] = [];
  for (const plan of plans) {
    const it = (assessment?.items ?? []).find((x: any) => x.appointmentId === plan.appointmentId);
    if (!it) continue;
    const chosen = plan.options.find((o: any) => o.id === plan.chosenOptionId) ?? plan.options[0];
    if (!chosen) continue;
    const origDoctor = getDoctor(m.doctorId ?? getAppt(plan.appointmentId).doctorId);
    items.push({
      patientId: it.patientId,
      patientName: it.patientName,
  ...
  const res = await runCommsDraft({ caseId, purpose: "reschedule_offer", items }, { caseId });
  const linted = bannedContentLint("reschedule_offer", items, res.output as CommsDraftResult);
  for (const w of linted.warnings) timeline(caseId, "comms", "error", "Draft replaced by safe template", w);

  let created = 0;
  for (const plan of plans) {
    if (!plan.options.length || plan.chosenOptionId === "none") {
      timeline(caseId, "recovery", "escalation", `No valid options for appointment ${plan.appointmentId} — staff attention needed`, plan.rationale);
      continue;
    }
    const it = (assessment?.items ?? []).find((x: any) => x.appointmentId === plan.appointmentId);
    const draft = linted.result.drafts.find((d) => d.appointmentId === plan.appointmentId);
    ...
    db.insert(schema.recommendations)
      .values({
        ...
        payload: {
          ...
          priorityReason: it?.priorityReason,
          tags: it?.tags ?? [],
          from: { doctorId: appt.doctorId, doctorName: getDoctor(appt.doctorId).name, startUtc: appt.startUtc, when: fmtWhen(appt.startUtc) },
      ...
      .run();
    created += 1;
    timeline(caseId, "comms", "recommendation", `Offer ready for ${it?.patientName}`, draft ? draft.subject : undefined, { appointmentId: plan.appointmentId });
  }
  if (opts?.replanOf) {
    db.update(schema.recommendations).set({ outcome: "superseded" }).where(eq(schema.recommendations.id, opts.replanOf)).run();
  }
  const c = getCase(caseId);
  if (c.state === "planning")
    transitionCase(caseId, "awaiting_approval", "orchestrator", `${created} recommendation${created === 1 ? "" : "s"} ready — nothing is sent or written until staff approve.`);
  await pace();
...
export async function nudgeStep(caseId: string, purpose: Extract<DraftPurpose, "confirm_nudge" | "preventive">): Promise<string> {
  const m = meta(caseId);
  const c = getCase(caseId);
  if (c.state === "open") transitionCase(caseId, "assessing", "orchestrator", "Reviewing appointment context");
  const appt = getAppt(m.appointmentId);
  const patient = getPatient(appt.patientId);
  const doctor = getDoctor(appt.doctorId);
  if (getCase(caseId).state === "assessing") transitionCase(caseId, "planning", "orchestrator", "Drafting outreach");
  await pace();
  const items: DraftItem[] = [
    {
      ...
      context: { doctorName: doctor.name, originalWhen: fmtWhen(appt.startUtc) },
    },
  ];
  const res = await runCommsDraft({ caseId, purpose, items }, { caseId });
  const linted = bannedContentLint(purpose, items, res.output as CommsDraftResult);
  for (const w of linted.warnings) timeline(caseId, "comms", "error", "Draft replaced by safe template", w);
  ...
  db.insert(schema.recommendations)
    .values({
      ...
      payload: {
        ...
        patientName: patient.name,
        type: appt.type,
        from: { doctorId: doctor.id, doctorName: doctor.name, startUtc: appt.startUtc, when: fmtWhen(appt.startUtc) },
        ...
        rationale:
          purpose === "confirm_nudge"
            ...
            : m.flag?.explanation ?? `${patient.name} shows elevated no-show risk; early outreach protects the slot.`,
        draft: { subject: draft.subject, body: draft.body },
      },
      createdAt: new Date().toISOString(),
    })
    .run();
  timeline(caseId, "comms", "recommendation", `${purpose === "confirm_nudge" ? "Confirmation nudge" : "Preventive outreach"} drafted for ${patient.name}`, draft.subject);
  transitionCase(caseId, "awaiting_approval", "orchestrator", "Draft ready — awaiting staff approval.");
  await pace();
...
export async function waitlistStep(caseId: string): Promise<string> {
  const m = meta(caseId);
  const c = getCase(caseId);
  if (c.state === "open") transitionCase(caseId, "assessing", "orchestrator", "Reviewing the vacated slot");
  ...
  if (getCase(caseId).state === "assessing") transitionCase(caseId, "planning", "orchestrator", "Ranking waitlist candidates");
  await pace();
  ...
  const res = await runWaitlistFill(
    { caseId, slot, slotType: appt.type as any, vacatedAppointmentId: appt.id, candidates },
    { caseId }
  );
  const fill: WaitlistFillResult = res.output;
  updateCaseMeta(caseId, { fill });
  if (fill.chosenWaitlistId === "none" || fill.candidates.length === 0) {
    timeline(caseId, "recovery", "escalation", "No waitlist match for the vacated slot", fill.rationale);
    transitionCase(caseId, "awaiting_approval", "orchestrator", "No candidate found — staff may close or handle manually.");
  ...
  const items: DraftItem[] = [
    {
      ...
      context: { proposedWhen: fmtWhen(slot.startUtc), proposedDoctorName: doctor.name },
    },
  ];
  const dres = await runCommsDraft({ caseId, purpose: "waitlist_offer", items }, { caseId });
  const linted = bannedContentLint("waitlist_offer", items, dres.output as CommsDraftResult);
  for (const w of linted.warnings) timeline(caseId, "comms", "error", "Draft replaced by safe template", w);
  const draft = linted.result.drafts[0];
  db.insert(schema.recommendations)
    .values({
    ...
    .run();
  timeline(caseId, "comms", "recommendation", `Waitlist offer ready for ${chosenPatient.name}`, draft.subject);
  transitionCase(caseId, "awaiting_approval", "orchestrator", "Backfill offer ready — awaiting staff approval.");
  await pace();
...
export async function replanSingle(
  ...
  args: {
    ...
    constraint: { afterTime?: string; beforeTime?: string; dayPart?: "am" | "pm"; preferredDay?: string };
    note: string;
  }
): Promise<string> {
  const c = getCase(caseId);
  if (c.state === "resolving") transitionCase(caseId, "planning", "orchestrator", `Replanning one patient — ${args.note}`);
  audit({ actor: "orchestrator", action: "case.replan", caseId, refType: "recommendation", refId: args.supersededRecId, detail: args.constraint });
  await scheduleStep(caseId, { appointmentId: args.appointmentId, ...args.constraint });
  await recoverStep(caseId, args.appointmentId);
  return commsStep(caseId, { replanOf: args.supersededRecId, replanNote: args.note, onlyAppointmentId: args.appointmentId });
}
