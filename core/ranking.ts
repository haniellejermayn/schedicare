 ...
 */
export function scoreRecoveryOption(ctx: RecoveryContext, slot: Slot, doctorName: string): ScoredSlot {
  ...
  const dayDelta = Math.abs(differenceInCalendarDays(new Date(slot.startUtc), new Date(ctx.originalStartUtc)));
  const soonness = Math.max(0, 25 - dayDelta * 5);
  chips.push({ label: dayDelta === 0 ? "Same day as original" : `${dayDelta} day${dayDelta > 1 ? "s" : ""} from original`, pts: soonness });
  ...
  if (slot.doctorId === ctx.originalDoctorId) {
    ...
    chips.push({ label: `Moves to ${doctorName}${slotBlock === origBlock ? " · same day-part" : ""}`, pts: fitPts });
  }
  if (ctx.patientPreferredDoctorId && slot.doctorId === ctx.patientPreferredDoctorId && slot.doctorId !== ctx.originalDoctorId) {
  ...
  chips.push({ label: `Keeps ${doctorName.split(" ").slice(-1)[0]}'s day at ${Math.round(load * 100)}% capacity`, pts: headroomPts });

  // w5 — waiting-time fairness (0..10).
  if (ctx.waitingSinceDays != null) {
    const fair = Math.min(10, Math.round(ctx.waitingSinceDays / 3));
    if (fair > 0) chips.push({ label: `Waiting ${ctx.waitingSinceDays} days`, pts: fair });
  }

  // w6 — staff priority (0..10).
  if (ctx.staffPriority > 0) chips.push({ label: `Staff priority ${ctx.staffPriority === 2 ? "high" : "elevated"}`, pts: ctx.staffPriority * 5 });

  // w7 — historical acceptance likelihood (0..8).
  const accept = ctx.acceptanceLikelihood ?? 0.6;
  const acceptPts = Math.round(accept * 8);
  chips.push({ label: `${Math.round(accept * 100)}% historical acceptance for similar offers`, pts: acceptPts });
...
export function rankRecoveryOptions(ctx: RecoveryContext, slots: Array<Slot & { doctorName: string }>): ScoredSlot[] {
  return slots
    .map((s) => scoreRecoveryOption(ctx, s, s.doctorName))
    .sort((a, b) => b.score - a.score || a.slot.startUtc.localeCompare(b.slot.startUtc));
}
...
export function rankWaitlistCandidates(
  ...
  return candidates
    ...
    .map((c) => {
      const chips: Array<{ label: string; pts: number }> = [];
      const waitDays = Math.max(0, differenceInCalendarDays(now, new Date(c.addedAt)));
      chips.push({ label: `Waiting ${waitDays} day${waitDays === 1 ? "" : "s"}`, pts: Math.min(30, waitDays * 2) });
      if (c.dayPart === "any" || c.dayPart === slot.block)
        chips.push({ label: c.dayPart === "any" ? "Flexible on time of day" : `Wants ${slot.block.toUpperCase()} — matches`, pts: 15 });
      else chips.push({ label: `Prefers ${c.dayPart.toUpperCase()} (slot is ${slot.block.toUpperCase()})`, pts: 0 });
      if (c.preferredDoctorId && c.preferredDoctorId === slot.doctorId)
        chips.push({ label: "Preferred doctor matches", pts: 12 });
      if (c.staffPriority > 0) chips.push({ label: `Staff priority ${c.staffPriority === 2 ? "high" : "elevated"}`, pts: c.staffPriority * 8 });
      const noShows = c.history.filter((h) => h.kind === "no_show").length;
      if (noShows === 0) chips.push({ label: "Reliable attendance history", pts: 8 });
      else chips.push({ label: `${noShows} prior no-show${noShows > 1 ? "s" : ""}`, pts: -6 * noShows });
      const score = chips.reduce((s, ch) => s + ch.pts, 0);
      return { ...c, score, dots: Math.max(1, Math.min(5, Math.round(score / 13))), chips };
    })
...
export function minutesFromOriginal(originalStartUtc: string, slot: Slot): number {
  return Math.abs(differenceInMinutes(new Date(slot.startUtc), new Date(originalStartUtc)));
}
